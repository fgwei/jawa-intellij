...
trait JawaTypePsiTypeBridge extends TypeSystemOwner {
  ...
  def toJawaType(`type`: PsiType,
    ...
    case arrayType: PsiArrayType => JawaType.addDimensions(JavaKnowledge.OBJECT, arrayType.getArrayDimensions)
...
//    case null => Any
  ...
  def toPsiType(`type`: JawaType,
    ...
    `type` match {
      case t if t == JavaKnowledge.OBJECT => javaObject
...
//    }
  ...
  protected def createJavaObject(project: Project, scope: GlobalSearchScope): PsiType = {
