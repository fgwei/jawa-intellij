...
object Packaging {
  ...
  def compressPackagedPlugin(source: File, destination: File): Unit =
    IO.zip((source.getParentFile ** AllPassFilter) pair (io.Path.relativeTo(source.getParentFile), false), destination)
  ...
  private def mergeIntoTemporaryJar(filesToMerge: File*): File =
    IO.withTemporaryDirectory { tmp =>
      ...
      zipFile.delete()
      IO.zip((tmp ** AllPassFilter) pair (io.Path.relativeTo(tmp), false), zipFile)
      zipFile
    }
}
